...
export class AuthService {
  ...
  constructor() {
    // Deferred to a microtask: initializeAuthState() can synchronously fire
    // an HTTP request (loadCurrentUser, when a token already exists), which
    // runs through authInterceptor - and that interceptor injects
    // AuthService. Calling it directly here would ask the injector for this
    // very instance while it's still being constructed, tripping Angular's
    // NG0200 circular-dependency guard (the request never reaches the
    // network - it fails in DI - and the error handler then clears the
    // valid token, forcing a login even though the session was fine).
    // Deferring past the current call stack lets the constructor return and
    // the singleton get cached first.
    queueMicrotask(() => this.initializeAuthState());
  }
